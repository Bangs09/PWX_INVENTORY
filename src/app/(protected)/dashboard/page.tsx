...
function formatActivityDate(iso: string) {
    ...
    } else if (dateStr && !dateStr.includes('Z') && !dateStr.includes('+')) {
        dateStr = dateStr + 'Z';
    }
    ...
    if (active && payload && payload.length) {
        ...
        return (
            <div className="rounded-xl bg-neutral-900 px-4 py-3 shadow-2xl border border-neutral-800 animate-in fade-in zoom-in-95 duration-200 min-w-[220px]">
                ...
                <div className="space-y-2 pr-1 text-left">
                    ...
                    {items.length > 5 && (
                        <p className="text-[9px] text-neutral-500 pt-1.5 text-center italic border-t border-neutral-800 mt-2">
                            + {items.length - 5} more items. Click card to view full list.
                        </p>
                    )}
                </div>
            </div>
...
export default function DashboardPage() {
    ...
    return (
        <div className="space-y-10 min-h-screen bg-transparent">
            ...
            {/* Stats Grid */}
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4 relative">
                {statsData.map((stat) => {
                    const CardComponent = (
                        <Card className="relative z-10 hover:z-50 rounded-[22px] border-neutral-100 bg-white shadow-[0_4px_20px_rgba(0,0,0,0.05)] transition-all duration-300 ease-in-out hover:border-neutral-200 hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] cursor-pointer hover:-translate-y-1 h-full flex flex-col py-3">
