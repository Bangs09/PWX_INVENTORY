...
function CreateBOMPageInner() {
    ...
    const isReadOnly = bomStatus === "Pending Approval" || bomStatus === "Approved";
    ...
    // Load BOM from server when opening /bom/create?edit=<id>
    useEffect(() => {
        if (!editParam) {
            setComponents([emptyComponent(1)]);
            return;
        ...
        async function loadBOM() {
            try {
                const entry = await fetchBOMByIdFromServer(editParam!);
                ...
                console.error("Failed to load BOM for editing:", error);
                toast.error(error.message || "Failed to load BOM for editing");
                router.push("/bom");
            }
        }
    ...
    const handleSave = useCallback(
        async (status: "Draft" | "Active") => {
            ...
            const newBom = {
                ...
                description,
                componentRows: filteredComponents.map(c => ({
                    ...
                    catalogSku: c.catalogSku,
                })),
                totalCost: rolledUpCost,
                status: editingId ? bomStatus : "Draft",
            };

            try {
                await saveBOMToServer(newBom);
            ...
            <div className="mx-auto max-w-7xl px-6 py-8">
                ...
                {currentStep === 2 && (
                    ...
                    <div className="space-y-6">
                        ...
                        {/* Component Data Grid */}
                        <fieldset disabled={isReadOnly} className="block border-0 p-0 m-0 w-full">
