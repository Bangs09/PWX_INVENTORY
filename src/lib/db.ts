...
// Dynamic DB initialization for BOMs and transactions
db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS inventory_transactions (
        ...
        transaction_type TEXT NOT NULL DEFAULT 'BOM_CONSUMPTION',
        reference_number TEXT NOT NULL, -- BOM ID
        component_id INTEGER REFERENCES inventory_components(id),
    ...
    CREATE INDEX IF NOT EXISTS idx_inventory_transactions_ref ON inventory_transactions(reference_number);
`);
...
export async function getBOMs(role?: string, userEmail?: string): Promise<BOM[]> {
    try {
        let query = "SELECT * FROM boms ORDER BY last_modified_at DESC";
        const boms = db.prepare(query).all() as BOM[];

        // Load lines count for each BOM
        for (const bom of boms) {
            const countRow = db.prepare("SELECT COUNT(*) as count FROM bom_items WHERE bom_id = ?").get(bom.id) as { count: number };
            (bom as any).components = countRow.count;
            (bom as any).lastModified = new Date(bom.last_modified_at).toLocaleDateString() + ' ' + new Date(bom.last_modified_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        }
        return boms;
    } catch (error) {
        console.error("Failed fetching BOMs:", error);
        throw new Error("Internal Database Error");
    }
}

export async function getBOMById(id: string): Promise<BOM | null> {
    try {
        const bom = db.prepare("SELECT * FROM boms WHERE id = ?").get(id) as BOM | undefined;
        if (!bom) return null;

        const items = db.prepare("SELECT * FROM bom_items WHERE bom_id = ? ORDER BY line_number ASC").all(id) as BOMItem[];
        bom.componentRows = items;
        (bom as any).components = items.length;
        (bom as any).lastModified = new Date(bom.last_modified_at).toLocaleDateString() + ' ' + new Date(bom.last_modified_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });

        return bom;
    } catch (error) {
        console.error("Failed fetching BOM by ID:", error);
        throw new Error("Internal Database Error");
    }
}

export async function saveBOM(bom: Partial<BOM> & { components?: Partial<BOMItem>[] }, email: string): Promise<BOM> {
    const bomId = bom.id || `BOM-${Date.now().toString().slice(-4)}`;
    
    const executeTx = db.transaction(() => {
        // If editing existing, verify it is in Draft or Rejected state
        const existing = db.prepare("SELECT status FROM boms WHERE id = ?").get(bomId) as { status: string } | undefined;
        if (existing && existing.status !== "Draft" && existing.status !== "Rejected") {
            throw new Error("Only Draft or Rejected BOMs can be edited.");
        }
        ...
        // Upsert BOM header
        db.prepare(`
            INSERT INTO boms (id, name, cpn, revision, phase, target_qty, description, total_cost, status, author, last_modified_by, last_modified_at)
            ...
            ON CONFLICT(id) DO UPDATE SET
                ...
                updated_at = CURRENT_TIMESTAMP
        `).run(bomId, name, cpn, revision, phase, target_qty, description, total_cost, status, author, email);
        ...
        const insertItem = db.prepare(`
            INSERT INTO bom_items (id, bom_id, line_number, level, part_number, description, qpa, uom, unit_cost, manufacturer, mpn, ref_designator, catalog_sku)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `);

        const componentRows = bom.componentRows || bom.components || [];
        componentRows.forEach((row: any, index: number) => {
            const itemId = row.id || `line-${bomId}-${index}-${Math.random().toString(36).substr(2, 4)}`;
            const lineNumber = row.lineNumber || index + 1;
            const level = row.level || 1;
            const partNumber = row.partNumber || row.part_number || "";
            const desc = row.description || "";
            const qpa = row.qpa || 0;
            const uom = row.uom || "Each";
            const unitCost = row.unitCost || row.unit_cost || 0;
            const manufacturer = row.manufacturer || null;
            const mpn = row.mpn || null;
            const refDes = row.refDesignator || row.ref_designator || null;
            const catalogSku = row.catalogSku || row.catalog_sku || null;

            insertItem.run(itemId, bomId, lineNumber, level, partNumber, desc, qpa, uom, unitCost, manufacturer, mpn, refDes, catalogSku);
        });

        // Fetch and return the newly saved BOM
        const savedBom = db.prepare("SELECT * FROM boms WHERE id = ?").get(bomId) as BOM;
        const savedItems = db.prepare("SELECT * FROM bom_items WHERE bom_id = ? ORDER BY line_number ASC").all(bomId) as BOMItem[];
        savedBom.componentRows = savedItems;
        return savedBom;
    });
    ...
    try {
        ...
        console.error("Failed saving BOM:", error.message);
        throw error;
    }
}
...
export async function deleteBOM(id: string): Promise<void> {
    const executeTx = db.transaction(() => {
        const existing = db.prepare("SELECT status FROM boms WHERE id = ?").get(id) as { status: string } | undefined;
        if (!existing) return;
        if (existing.status !== "Draft" && existing.status !== "Rejected" && existing.status !== "Archived") {
            throw new Error("BOM cannot be deleted in its current status.");
        }
    ...
    try {
        ...
        console.error("Failed deleting BOM:", error);
        throw error;
    }
}

export async function submitBOMForApproval(id: string, email: string): Promise<BOM> {
    const executeTx = db.transaction(() => {
        const bom = db.prepare("SELECT * FROM boms WHERE id = ?").get(id) as BOM | undefined;
        if (!bom) throw new Error("BOM not found");
        if (bom.status !== "Draft" && bom.status !== "Rejected") {
            throw new Error("BOM must be in Draft or Rejected state to submit for approval.");
        }
        ...
        // Notify admins
        const admins = db.prepare("SELECT id FROM users WHERE role IN ('admin', 'co-admin')").all() as { id: number }[];
        for (const admin of admins) {
            db.prepare(`
                INSERT INTO notifications (user_id, message, type, related_id)
                VALUES (?, ?, 'bom_submission', ?)
            `).run(admin.id, `New BOM awaiting approval. BOM Number: ${bom.id}. Submitted By: ${email}`, bom.id);
        }

        // Return updated BOM
        return db.prepare("SELECT * FROM boms WHERE id = ?").get(id) as BOM;
    });
    ...
    try {
        ...
        console.error("Failed submitting BOM:", error);
        throw error;
    }
}

export async function approveBOM(id: string, adminEmail: string): Promise<BOM> {
    const executeTx = db.transaction(() => {
        const bom = db.prepare("SELECT * FROM boms WHERE id = ?").get(id) as BOM | undefined;
        if (!bom) throw new Error("BOM not found");
        ...
        const items = db.prepare("SELECT * FROM bom_items WHERE bom_id = ?").all(id) as BOMItem[];
        const targetQty = bom.target_qty;

        // Stock validation: check all components first
        const deductions: { compId: number; componentName: string; requiredQty: number; availableQty: number; stockBefore: number; warehouse: string }[] = [];

        for (const item of items) {
            const requiredQty = Math.round(item.qpa * targetQty);
            const sku = item.part_number.toUpperCase().trim();

            if (!sku) continue; // Skip lines without part number

            // Lookup component in PWX warehouse to find if it has sufficient stock
            ...
            `).all(sku) as { id: number; name: string; stock: number; warehouse: string }[];
            ...
            // Find a warehouse that has stock >= requiredQty
            const match = warehousesWithStock.find(w => w.stock >= requiredQty);

            if (!match) {
                // Insufficient stock. Calculate total available across all warehouses to report
        ...
        // Perform stock deduction and log transactions
        for (const ded of deductions) {
            ...
            `).run(id, ded.compId, ded.requiredQty, ded.stockBefore, stockAfter, adminEmail);
        }
        ...
        db.prepare(`
            ...
            SET status = 'Approved',
                ...
                updated_at = CURRENT_TIMESTAMP
            WHERE id = ?
        `).run(adminEmail, adminEmail, id);

        // Notify creator
        const creator = db.prepare("SELECT id FROM users WHERE email = ?").get(bom.author) as { id: number } | undefined;
        if (creator) {
            db.prepare(`
                INSERT INTO notifications (user_id, message, type, related_id)
                VALUES (?, ?, 'bom_approval', ?)
            `).run(creator.id, `Your BOM has been approved. BOM Number: ${id}. Inventory has been deducted successfully.`, id);
        }
    ...
    try {
        ...
        console.error("Failed approving BOM:", error);
        throw error;
    }
}

export async function rejectBOM(id: string, remarks: string, adminEmail: string): Promise<BOM> {
    const executeTx = db.transaction(() => {
        const bom = db.prepare("SELECT * FROM boms WHERE id = ?").get(id) as BOM | undefined;
        if (!bom) throw new Error("BOM not found");
        if (bom.status !== "Pending Approval") {
            throw new Error("BOM must be Pending Approval to be rejected.");
        }
        ...
        db.prepare(`
            ...
            SET status = 'Rejected',
                ...
                updated_at = CURRENT_TIMESTAMP
            WHERE id = ?
        `).run(adminEmail, remarks, adminEmail, id);
        ...
        if (creator) {
            db.prepare(`
                INSERT INTO notifications (user_id, message, type, related_id)
                VALUES (?, ?, 'bom_rejection', ?)
            `).run(creator.id, `Your BOM has been rejected. Reason: ${remarks}`, id);
        }
    ...
    try {
        ...
        console.error("Failed rejecting BOM:", error);
        throw error;
    }
}

export async function getBOMTransactions(bomId: string): Promise<InventoryTransaction[]> {
    try {
        const query = `
            SELECT t.*, c.name as component_name, c.sku as component_sku, c.warehouse as component_warehouse
            FROM inventory_transactions t
            JOIN inventory_components c ON t.component_id = c.id
            WHERE t.reference_number = ?
            ORDER BY t.created_at DESC
        `;
        return db.prepare(query).all(bomId) as InventoryTransaction[];
    } catch (error) {
        console.error("Failed fetching BOM transactions:", error);
        throw new Error("Internal Database Error");
    }
}

export async function archiveBOM(id: string, email: string): Promise<BOM> {
    const executeTx = db.transaction(() => {
        const bom = db.prepare("SELECT * FROM boms WHERE id = ?").get(id) as BOM | undefined;
        if (!bom) throw new Error("BOM not found");
        
        const nextStatus = bom.status === "Archived" ? "Draft" : "Archived";
    ...
    try {
        ...
        console.error("Failed archiving BOM:", error);
        throw error;
    }
}
