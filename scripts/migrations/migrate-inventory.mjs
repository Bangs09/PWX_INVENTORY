...
async function migrate() {
    try {
        ...
        const insertComponent = db.prepare("INSERT INTO inventory_components (sku, name, stock, min_stock, category, warehouse) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT (sku, warehouse) DO UPDATE SET stock = EXCLUDED.stock");
        
        for (const item of componentsSeed) {
            // Seed for both warehouses
            insertComponent.run(item.sku, item.name, item.stock, item.min, item.category, "PWX IoT Hub");
            insertComponent.run(item.sku, item.name, item.stock, item.min, item.category, "Jenny's");
        }

        // 2. Seed Gateways
        console.log("Seeding Gateways...");
        const insertGateway = db.prepare("INSERT INTO gateways (name, sku, location, quantity) VALUES (?, ?, ?, ?) ON CONFLICT (sku) DO NOTHING");
        
        for (const gw of gatewaysSeed) {
            insertGateway.run(gw.id, gw.sku, gw.location, gw.quantity);
        }
